...
summary_ICC_UCM <- 
  ...
  mutate(GA = (GA_weeks*7 + (GA_days)) %>% as.numeric(), 
         mat_ID = if_else(is.na(mat_ID), 
...
#mortality_ICC_UCM_count <-mortality_ICC_UCM_count %>% filter(!Trial_id %in% double_zero_trials)
...
china_xie_2020_two <-
  logistf (
    death_discharge_new ~ treatment_cat * GA,
    data = ICC_UCM [ICC_UCM$Trial_id == "china_xie_2020", ],
    family = binomial("logit")
  ) %>% tidy() %>% mutate(Trial_id = "china_xie_2020")
```
...
ireland_finn_2019_two <-
  logistf(
    death_discharge_new ~ treatment_cat * GA,
    data = ICC_UCM [ICC_UCM$Trial_id == "ireland_finn_2019", ],
    family = binomial("logit"),
    plcontrol = logistpl.control(maxit=1000),
    control = logistf.control(maxit=100)
  ) %>% tidy() %>% mutate(Trial_id = "ireland_finn_2019")
```
...
japan_hosono_2008_two <-
  logistf(
    death_discharge_new ~ treatment_cat +
      GA,
    data = ICC_UCM [ICC_UCM$Trial_id == "japan_hosono_2008", ],
    family = binomial("logit")
  ) %>% tidy() %>% mutate(Trial_id = "japan_hosono_2008")
```
...
taiwan_shen_2018_two <-
  logistf(
    death_discharge_new ~ treatment_cat +
      GA,
    data = ICC_UCM [ICC_UCM$Trial_id == "taiwan_shen_2018", ],
    family = binomial("logit")
  ) %>% tidy() %>% mutate(Trial_id = "taiwan_shen_2018")
```
...
thailand_tanthawat_2016_two <-
  logistf(
    death_discharge_new ~ treatment_cat * GA,
    data = ICC_UCM [ICC_UCM$Trial_id == "thailand_tanthawat_2016", ],
    family = binomial("logit"),
    # plcontrol = logistpl.control(maxit=1000),
    control = logistf.control(maxit=100)
  ) %>% tidy() %>% mutate(Trial_id = "thailand_tanthawat_2016")
```
...
turkey_alan_2014_two <-
  ...
  logistf(death_discharge_new ~ treatment_cat  * GA,
          ...
          family = binomial("logit"),
          control = logistf.control(maxit=100)
          # control = geese.control(maxit = 200)
  ) %>%
  tidy() %>% 
...
usa_josephsen_2014_two <-
  logistf(
    death_discharge_new ~ treatment_cat * GA,
    data = ICC_UCM [ICC_UCM$Trial_id == "usa_josephsen_2014", ],
    family = binomial("logit")
  ) %>% tidy() %>% mutate(Trial_id = "usa_josephsen_2014")
```
...
usa_katheria_2014_two <-
  logistf (
    death_discharge_new ~ treatment_cat * GA,
    data = ICC_UCM [ICC_UCM$Trial_id == "usa_katheria_2014", ],
    family = binomial("logit")
  ) %>% tidy() %>% mutate(Trial_id = "usa_katheria_2014")
```
...
usa_march_2011_two <-
  logistf(
    death_discharge_new ~ treatment_cat * GA,
    data = ICC_UCM [ICC_UCM$Trial_id == "usa_march_2011", ],
    family = binomial("logit")
  ) %>% 
  tidy() %>%
  mutate(Trial_id = "usa_march_2011")
```
...
usa_mercer_2018_two <-
  logistf(
    death_discharge_new ~ treatment_cat * GA,
    data = ICC_UCM [ICC_UCM$Trial_id == "usa_mercer_2018", ],
    family = binomial("logit")
  ) %>% tidy() %>% mutate(Trial_id = "usa_mercer_2018")
...
res.IPD <- rma(yi, vi,  data = IPD_icc_UCM_death_df, method="FE",
         ai= n_events_UCM,
           ...
           di = n_ICC  - n_event_ICC, 
           n2i = n_ICC, 
           slab=paste((IPD_icc_UCM_death_df$Trial_id), sep=" "))
...
final_df1$GA_total_days <- ifelse(!is.na(final_df1$GA_days), 
                                  final_df1$GA_weeks*7 + final_df1$GA_days, final_df1$GA_weeks+3)
...
final_df1 %>% select(Trial_id,GA_weeks,GA_days,GA_total_days) %>% 
  filter(is.na(GA_days)) %>% 
  view()
```
...
final_df1 <- final_df1_ICC_UCM

#final_df1 <- final_df1_ICC_UCM

#final_df1 <- final_df1_UCM_UCM 
```
...
remove_trials_w_00_events(death_discharge_new)


final_df2 <- 
  final_df2 %>%
  group_by(Trial_id) %>%
  mutate(ICC_UCM_centered_within =
           ICC_UCM - mean(ICC_UCM)) %>% 
  ungroup
...
one_stage_model <- one_stage_glmm_ICC_UCM  %>% 
  ...
  mutate(across(where(is.numeric),
                ~round(.,digit= 2))) %>%
  filter (term ==	"ICC_UCM:GA_total_days_centered_within")
...
```{r}
final_df2 %>% tabyl(ICC_UCM,death_discharge_new)
```


```{r}
final_df2 %>% skimr::skim(GA_weeks)
```


```{r fig.height= 10, fig.width=18, dpi=500}
...
sav <- forest(res.IPD, atransf=exp, addpred=TRUE, rows = c(1:(data_height)), xlab ="Interaction estimate (log scale)",mlab=mlabfun("Adjusted Two Stage FE Model", res.IPD), order=IPD_icc_UCM_death_df$alloc, 
                ylim=c(-2.3,(data_height +3)), xlim=c(left_bound,right_bound),
                alim = c(log(0.5), log(2)), cex=1.3, showweights = TRUE,
                ilab=cbind(paste(IPD_icc_UCM_death_df$ai, "/", IPD_icc_UCM_death_df$n1i), paste(IPD_icc_UCM_death_df$ci, "/", IPD_icc_UCM_death_df$n2i)), ilab.xpos=seq(-4.8,-3.8, length = 2), slab =res.IPD$slab,  header = FALSE, annotate = TRUE)
...
text(left_bound, data_height+3.5, pos=4, cex=1.3, c("UCM vs ICC Meta-analysis - Death before discharge (treatment covariate interaction gestational age)"), font = 4)
  
  text(sav$ilab.xpos, (data_height+2), pos=1, c("(n/N)","(n/N)"), cex = 1.1, font =3)
  
text(c(mean(sav$ilab.xpos[1:1.5]),mean(sav$ilab.xpos[2:2.5])), data_height+3, c("UCM","ICC"), pos=1, cex=1.3)

text(c(log(0.05),log(20)), -2.2, pos=1, c("Favours UCM", "Favours ICC"), cex=1.3)
text(6,                     24, "Interaction estimate", cex=1.3, pos = 2)
text(6,                     23, "[95% CI]", cex=1.3, pos = 2)
text(-7.8,                     23, "Study", cex=1.3, pos = 2)
text(4.4,                     23,"Weight", cex=1.3, pos = 2)
