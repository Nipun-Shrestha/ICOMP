...
final_df1_ICC_DCC <- final_df1_ICC_DCC %>%  group_by(Trial_id) %>%  filter(sum(ICC_DCC == 1) > 0 & 
           sum(ICC_DCC == 2) > 0) 
...
```{r}
one_stage_glmm_ICC_DCC %>% summary
```
Odds ratios results:
```{r}
one_stage_glmm_ICC_DCC %>% 
...
sav <- forest(res.IPD, atransf=exp, addpred=TRUE, rows = c(1:(data_height)), xlab ="Odds ratio (log scale)",mlab=mlabfun("Adjusted Two Stage FE Model", res.IPD), order=IPD_icc_dcc_death_df$alloc, 
                ylim=c(-2.3,(data_height +3)), xlim=c(left_bound,right_bound),
                alim = c(log(0.05), log(20)), cex=1.3, showweights = TRUE,
                ilab=cbind(paste(counts$ai, "/", counts$n1i), paste(counts$ci, "/", counts$n2i)), ilab.xpos=seq(-4.8,-3.8, length = 2), slab =res.IPD$slab,  header = FALSE, annotate = TRUE)
...
par(xpd=NA)
  ...
  text(left_bound, data_height+3.5, pos=4, cex=1.3, c("ICC vs DCC Meta-analysis - Death before discharge"), font = 4)
  
  text(sav$ilab.xpos, (data_height+2), pos=1, c("(n/N)","(n/N)"), cex = 1.1, font =3)
  
text(c(mean(sav$ilab.xpos[1:1.5]),mean(sav$ilab.xpos[2:2.5])), data_height+3, c("ICC","DCC"), pos=1, cex=1.3)

text(c(log(0.05),log(20)), -2.2, pos=1, c("Favours DCC", "Favours ICC"), cex=1.3)
text(6,                     24, "Odds Ratio", cex=1.3, pos = 2)
text(6,                     23,"[95% CI]", cex=1.3, pos = 2)
text(4.5,                     23,"Weight", cex=1.3, pos = 2)
